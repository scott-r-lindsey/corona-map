...
appendZeroes = () => {
  Object.keys(counties).map((fips, i) => {
    countyData.counties[fips].series.confirmed.push(0)
    countyData.counties[fips].series.deaths.push(0)
  });
}
...
(async () => {
  ...
  for (let filename of csvs) {
    ...
    fileParsed.forEach((d) => {
      ...
      if (d['FIPS']){
        ...
        if (undefined !== countyData.counties[fips]){
          countyData.counties[fips].series.confirmed[i] = Math.trunc(d.Confirmed);
          countyData.counties[fips].series.deaths[i] = Math.trunc(d.Deaths);
        }
      }
  ...
  fs.writeFileSync(outputPath + 'full-county.json', JSON.stringify(countyData));
